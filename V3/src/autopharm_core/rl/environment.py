...
class MockPlantSimulator:
    ...
    def step(self, cpps: Dict[str, float]) -> Dict[str, float]:
        ...
        lod_response = -(cpps['spray_rate'] - 120) * 0.01 + (cpps['carousel_speed'] - 30) * 0.05 + 2.0
...
class GranulationEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if (obs[0] < 50 or obs[0] > 800 or  # d50 out of safe range
            obs[1] < 0.1 or obs[1] > 5.0):   # LOD out of safe range
            terminated = True
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            obs = self._get_obs()
            print(f"Step {self.current_step}: d50={obs[0]:.1f} (target: {obs[2]:.1f}), "
