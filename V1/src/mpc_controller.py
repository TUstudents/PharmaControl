...
class MPCController:
    ...
    def _filter_by_constraints(self, candidates, current_cpps):
        ...
        for seq in candidates:
            ...
            for i, name in enumerate(cpp_names):
                ...
                if not (
                    self.constraints[name]["min_val"]
                    <= first_step[i]
                    <= self.constraints[name]["max_val"]
                ):
                    is_valid = False
                    break
    ...
    def _calculate_cost(self, prediction, action, target_cmas, current_action_scaled):
        ...
        proposed_first_action_scaled = action[
            0, 0, : len(self.config["cpp_names"])
        ]  # Only base CPPs, not soft sensors
    ...
    def suggest_action(
        self, past_cmas: pd.DataFrame, past_cpps: pd.DataFrame, target_cmas: np.ndarray
    ) -> np.ndarray:
        ...
        valid_candidates_unscaled = self._filter_by_constraints(
            candidates_unscaled, current_cpps_unscaled
        )
        ...
        target_cmas_tensor = (
            torch.tensor(target_cmas_scaled, dtype=torch.float32).unsqueeze(0).to(self.device)
        )
        ...
        # Convert to tensors
        past_cmas_tensor = (
            torch.tensor(past_cmas_values, dtype=torch.float32).unsqueeze(0).to(self.device)
        )
        past_cpps_tensor = (
            torch.tensor(past_cpps_values, dtype=torch.float32).unsqueeze(0).to(self.device)
        )
        ...
        try:
            ...
            froude_number_idx = self.config["cpp_names_and_soft_sensors"].index(
                "froude_number_proxy"
            )
        except ValueError as e:
            raise ValueError(
                f"Required variable not found in configuration: {e}. "
        ...
        with torch.no_grad():
            ...
            for action_seq_unscaled in pbar:
                ...
                action_seq_with_sensors = np.zeros(
                    (action_seq_unscaled.shape[0], len(self.config["cpp_names_and_soft_sensors"]))
                )
                ...
                action_seq_with_sensors[:, specific_energy_idx] = (
                    spray_rate * carousel_speed
                ) / 1000.0  # specific_energy
                action_seq_with_sensors[:, froude_number_idx] = (
                    carousel_speed**2
                ) / 9.81  # froude_number_proxy

                # 2. Scale the full feature sequence
                action_df_unscaled = pd.DataFrame(
                    action_seq_with_sensors, columns=self.config["cpp_names_and_soft_sensors"]
                )
                ...
                action_tensor = (
                    torch.tensor(action_df_scaled.values, dtype=torch.float32)
                    .unsqueeze(0)
                    .to(self.device)
                )
                ...
                cost = self._calculate_cost(
                    prediction, action_tensor, target_cmas_tensor, current_cpps_tensor
                )
        ...
        if best_action_sequence is None:
            print(
                "Warning: No valid control actions found - all candidates produced invalid costs."
            )
            print(
                f"Evaluated {len(valid_candidates_unscaled)} candidates, final best_cost: {best_cost}"
            )
