...
def demonstrate_mock_vs_real_history():
    ...
    print(f"- Particle size (d50) responds from {measurement_trajectory[0, 0]:.1f} to {measurement_trajectory[-1, 0]:.1f} μm")
    print()
    
    # Phase 1: Build up real history
    print("Phase 1: Building real trajectory history...")
    for i, (control, measurement) in enumerate(zip(control_trajectory[:20], measurement_trajectory[:20])):
    ...
    print(f"Current state: d50={current_measurement[0]:.1f}μm, spray_rate={current_control[0]:.1f}g/min")
    print()
    ...
    print(f"  - Reflects actual trajectory: {control_trajectory[5, 0]:.1f} → {control_trajectory[19, 0]:.1f} g/min")
