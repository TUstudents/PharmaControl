...
def test_column_order_robustness():
    ...
    for i, test_config in enumerate(test_configurations, 1):
        ...
        try:
            ...
            for col in config["cpp_full_names"]:
                scaler = scalers[col]
                result_df[col] = scaler.inverse_transform(
            ...
            # Verify correctness
            specific_energy_correct = np.isclose(
                calc_specific_energy, expected_specific_energy, rtol=1e-3
            )
            froude_number_correct = np.isclose(
                calc_froude_number, expected_froude_number, rtol=1e-3
            )

            if specific_energy_correct and froude_number_correct:
                print(
                    f"   ✅ CORRECT: specific_energy={calc_specific_energy:.2f}, froude_number={calc_froude_number:.2f}"
                )
            else:
                print(
                    f"   ❌ INCORRECT: specific_energy={calc_specific_energy:.2f}, froude_number={calc_froude_number:.2f}"
                )
    ...
    config_missing_base["cpp_full_names"] = [
        "air_flow",
        "carousel_speed",
        "specific_energy",
        "froude_number_proxy",
    ...
    config_missing_soft["cpp_full_names"] = [
        "spray_rate",
        "air_flow",
        "carousel_speed",
        "froude_number_proxy",
    ...
    if all_results_correct:
        print(
            "🎉 SUCCESS: All tests passed! Soft sensor calculations are robust to column order changes."
        )
