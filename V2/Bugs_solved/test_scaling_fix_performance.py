...
class BuggyParameterBoundsController(RobustMPCController):
    ...
    def _get_fitness_function(self, past_cmas_scaled, past_cpps_scaled, target_cmas_unscaled):
        ...
        def fitness(control_plan_unscaled):
            ...
            past_cpps_tensor = np.zeros(
                (1, self.config["lookback"], len(self.config["cpp_full_names"]))
            )
...
def demonstrate_scaling_fix():
    ...
    print(
        f"  Buggy bounds range: {min(b[0] for b in buggy_bounds):.1f} to {max(b[1] for b in buggy_bounds):.1f}"
    )
    ...
    try:
        ...
        print(
            f"  Buggy action: [{buggy_action[0]:.1f}, {buggy_action[1]:.1f}, {buggy_action[2]:.1f}]"
        )
    ...
    print(
        f"  Fixed bounds range: {min(b[0] for b in fixed_bounds):.3f} to {max(b[1] for b in fixed_bounds):.3f}"
    )
    ...
    try:
        ...
        print(
            f"  Fixed action: [{fixed_action[0]:.1f}, {fixed_action[1]:.1f}, {fixed_action[2]:.1f}]"
        )
    ...
    if buggy_bounds and fixed_bounds:
        ...
        for i, (name, constraints) in enumerate(config["cpp_constraints"].items()):
            ...
            scaling_correct = np.isclose(fixed_min, expected_min) and np.isclose(
                fixed_max, expected_max
            )
    ...
    def check_constraints(action, config):
        ...
        for i, name in enumerate(config["cpp_names"]):
            ...
            if not (constraints["min_val"] <= value <= constraints["max_val"]):
                return (
                    False,
                    f"{name}={value:.1f} outside [{constraints['min_val']}, {constraints['max_val']}]",
                )
