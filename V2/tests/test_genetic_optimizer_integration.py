...
class TestEndToEndOptimization:
    ...
    def test_basic_optimization_workflow(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_optimization_with_linear_fitness(self, simple_ga_config, simple_param_bounds, linear_fitness_function):
        ...
        assert avg_speed < speed_center, f"Carousel speed {avg_speed} should be < center {speed_center}"
        
    def test_optimization_convergence_behavior(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_optimization_with_constraints(self, simple_ga_config, simple_param_bounds, constrained_fitness_function):
        ...
        assert np.all(spray_rates >= 100.0), f"Spray rate constraint violated: min = {np.min(spray_rates)}"
        
        # Constraint 2: air_flow < 600
        air_flows = result[:, 1]
        assert np.all(air_flows <= 600.0), f"Air flow constraint violated: max = {np.max(air_flows)}"
        
    def test_optimization_with_multimodal_fitness(self, simple_ga_config, simple_param_bounds, multimodal_fitness_function):
...
class TestPharmaceuticalScenarios:
    ...
    def test_granulation_startup_control(self, pharmaceutical_scenarios, pharmaceutical_ga_config):
        ...
        for _ in range(horizon):
            for param_name in ['spray_rate', 'air_flow', 'carousel_speed']:
                param_bounds.append((
                    constraints[param_name]['min_val'],
                    constraints[param_name]['max_val']
                ))
        ...
        optimizer = GeneticOptimizer(
            fitness_function=startup_fitness,
    ...
    def test_disturbance_rejection_scenario(self, pharmaceutical_scenarios, pharmaceutical_ga_config):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=disturbance_fitness,
            param_bounds=param_bounds,
            config=config
        )
    ...
    def test_grade_changeover_scenario(self, pharmaceutical_scenarios, pharmaceutical_ga_config):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=changeover_fitness,
            param_bounds=param_bounds,
            config=config
        )
...
class TestOptimizationRobustness:
    ...
    def test_optimization_with_noisy_fitness(self, simple_ga_config, simple_param_bounds, noisy_fitness_function):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=noisy_fitness_function,
            param_bounds=simple_param_bounds,
            config=config
        )
    ...
    def test_optimization_with_tight_bounds(self, simple_ga_config, quadratic_fitness_function, numerical_edge_cases):
    ...
    def test_optimization_with_large_bounds(self, simple_ga_config, quadratic_fitness_function, numerical_edge_cases):
    ...
    def test_optimization_determinism_with_seeding(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
...
class TestOptimizationPerformance:
    ...
    def test_optimization_completes_in_reasonable_time(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_fitness_function_call_count(self, simple_ga_config, simple_param_bounds):
        ...
        assert call_count <= expected_calls * 2, f"Too many fitness calls: {call_count} (expected ~{expected_calls})"


class TestConfigurationVariations:
    """Test class for different optimization configurations."""
    
    def test_small_population_optimization(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_large_population_optimization(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_many_generations_optimization(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
    ...
    def test_extreme_probability_configurations(self, simple_ga_config, simple_param_bounds, quadratic_fitness_function):
