...
class TestSafeFallbackInitialization:
    ...
    def test_fallback_action_initialized_on_startup(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        assert controller._last_successful_action is not None, \
            "Fallback action should be initialized, not None"
        
        # Verify it's a proper numpy array
        assert isinstance(controller._last_successful_action, np.ndarray), \
            "Fallback action should be numpy array"
        
        # Verify correct dimensions
        expected_size = len(test_config['cpp_names'])
        assert controller._last_successful_action.shape == (expected_size,), \
            f"Fallback action shape should be ({expected_size},), got {controller._last_successful_action.shape}"
        
        # Verify all values are finite
        assert np.all(np.isfinite(controller._last_successful_action)), \
            "All fallback action values should be finite"
    
    def test_safe_default_action_values(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        expected_values = np.array([expected_spray_rate, expected_air_flow, expected_carousel_speed])
        
        # Verify calculated values match expected midpoints
        np.testing.assert_array_equal(
            fallback_action, expected_values,
            err_msg="Safe default action should use constraint midpoints"
        )
    
    def test_calculate_safe_default_action_method(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        for i, name in enumerate(test_config['cpp_names']):
            ...
            assert np.isclose(value, expected_midpoint), \
                f"{name} should be midpoint {expected_midpoint}, got {value}"
    
    def test_first_step_optimizer_failure_safety(self, mock_model, mock_estimator, test_config, test_scalers):
    ...
    def test_fallback_action_validation(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        assert is_valid, \
            "Pre-initialized fallback action should pass controller validation"
    ...
    def test_missing_constraint_configuration_error(self, mock_model, mock_estimator, test_scalers):
        ...
        with pytest.raises(ValueError, match="Missing constraint configuration for CPP 'carousel_speed'"):
            RobustMPCController(
                model=mock_model,
                estimator=mock_estimator,
                optimizer_class=None,
    ...
    def test_invalid_constraint_bounds_error(self, mock_model, mock_estimator, test_scalers):
        ...
        with pytest.raises(ValueError, match="Invalid constraint bounds for 'spray_rate'.*min_val=200.0 >= max_val=80.0"):
            RobustMPCController(
                model=mock_model,
                estimator=mock_estimator,
                optimizer_class=None,
    ...
    def test_get_fallback_action_uses_new_method(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        np.testing.assert_array_equal(
            fallback_action, expected_action,
            err_msg="Fallback action should use safe default calculation method"
        )
    
    def test_fallback_strategy_hierarchy(self, mock_model, mock_estimator, test_config, test_scalers):
