...
class TestSoftSensorRobustness:
    ...
    def test_original_column_order(self, mock_model, mock_estimator, base_config, test_scalers):
        ...
        test_plan = np.array([
            [120.0, 500.0, 30.0],  # spray_rate, air_flow, carousel_speed
            [150.0, 600.0, 35.0]
        ])
    ...
    def test_reversed_column_order(self, mock_model, mock_estimator, base_config, test_scalers):
        ...
        config = base_config.copy()
        config['cpp_full_names'] = ['froude_number_proxy', 'specific_energy', 'carousel_speed', 'air_flow', 'spray_rate']
    ...
    def test_random_column_order(self, mock_model, mock_estimator, base_config, test_scalers):
        ...
        # Scrambled order
        config = base_config.copy()
        config['cpp_full_names'] = ['air_flow', 'specific_energy', 'spray_rate', 'froude_number_proxy', 'carousel_speed']
    ...
    def test_missing_required_base_variable(self, mock_model, mock_estimator, base_config, test_scalers):
        ...
        config = base_config.copy()
        config['cpp_full_names'] = ['air_flow', 'carousel_speed', 'specific_energy', 'froude_number_proxy']
    ...
    def test_soft_sensor_base_variable_validation(self, mock_model, mock_estimator, base_config, test_scalers):
    ...
    def test_missing_required_soft_sensor(self, mock_model, mock_estimator, base_config, test_scalers):
        """Test error handling when soft sensor variables are missing."""
        # Missing specific_energy - should fail at initialization
        config = base_config.copy()
        config['cpp_full_names'] = ['spray_rate', 'air_flow', 'carousel_speed', 'froude_number_proxy']
        
        with pytest.raises(ValueError, match="Missing required soft sensor variables.*specific_energy"):
            RobustMPCController(
                model=mock_model,
                estimator=mock_estimator,
                optimizer_class=None,
    ...
    def test_soft_sensor_calculation_accuracy(self, mock_model, mock_estimator, base_config, test_scalers):
    ...
    def test_cpp_not_in_full_names_error(self, mock_model, mock_estimator, base_config, test_scalers):
