...
class TestParameterBoundsScaling:
    ...
    def test_parameter_bounds_are_scaled(self, mock_model, mock_estimator, test_config, test_scalers):
    ...
    def test_parameter_bounds_scaling_correctness(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        assert np.isclose(spray_min_actual, spray_min_expected), \
            f"Spray rate min: expected {spray_min_expected}, got {spray_min_actual}"
        assert np.isclose(spray_max_actual, spray_max_expected), \
        ...
        assert np.isclose(air_min_actual, air_min_expected), \
            f"Air flow min: expected {air_min_expected}, got {air_min_actual}"
        assert np.isclose(air_max_actual, air_max_expected), \
            f"Air flow max: expected {air_max_expected}, got {air_max_actual}"
    
    def test_unscale_cpp_plan_correctness(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        for i, cpp_name in enumerate(test_config['cpp_names']):
            ...
            assert np.allclose(unscaled_plan[:, i], manual_unscaled), \
                f"Unscaling incorrect for {cpp_name}"
    
    def test_scaling_roundtrip_consistency(self, mock_model, mock_estimator, test_config, test_scalers):
        ...
        assert np.allclose(original_plan, roundtrip_plan, atol=1e-10), \
            "Scaling roundtrip not consistent"
    
    def test_optimizer_with_scaled_bounds(self, mock_model, mock_estimator, test_config, test_scalers):
    ...
    def test_fitness_function_with_scaled_inputs(self, mock_model, mock_estimator, test_config, test_scalers):
