...
class TestDataBufferRaceConditions:
    ...
    def test_deprecated_methods_warnings(self, test_buffer):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert "add_measurement() is deprecated due to race condition vulnerability" in str(
                w[0].message
            )
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(w) == 1
            assert issubclass(w[0].category, DeprecationWarning)
            assert "add_control_action() is deprecated due to race condition vulnerability" in str(
                w[0].message
            )
            assert "add_sample" in str(w[0].message)
    ...
    def test_simulated_race_condition_scenario(self, test_buffer):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert any(
                "CRITICAL: Buffer misalignment detected" in str(warning.message) for warning in w
            )
        ...
        if cma_len != cpp_len:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                try:
                    ...
                    assert any(
                        "CRITICAL: Buffer misalignment detected" in str(warning.message)
                        for warning in w
                    )
                except ValueError:
    ...
    def test_pharmaceutical_manufacturing_scenario(self, test_buffer):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            ), "Pharmaceutical manufacturing must have perfect data integrity"
