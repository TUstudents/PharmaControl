...
class TestPerformanceScaling:
    ...
    def test_execution_time_scaling_with_population_size(
        self, performance_configs, benchmark_functions
    ):
        ...
        for pop_size in population_sizes:
            ...
            optimizer = GeneticOptimizer(
                fitness_function=fitness_func, param_bounds=bounds, config=config
            )
        ...
        time_ratios = [
            execution_times[i + 1] / execution_times[i] for i in range(len(execution_times) - 1)
        ]
    ...
    def test_execution_time_scaling_with_generations(
        self, performance_configs, benchmark_functions
    ):
    ...
    def test_execution_time_scaling_with_problem_size(self, benchmark_functions):
        ...
        assert all(
            t < 30.0 for t in execution_times
        ), f"Some execution times too long: {execution_times}"
...
class TestMemoryUsage:
    ...
    def test_memory_cleanup_after_optimization(self):
        ...
        print(
            f"Memory: baseline={memory_baseline:.1f}MB, during={memory_during:.1f}MB, after={memory_after:.1f}MB"
        )
...
class TestConvergencePerformance:
    ...
    def test_optimization_with_expensive_fitness_function(self):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=expensive_fitness, param_bounds=bounds, config=config
        )
        ...
        assert (
            expected_min_time < execution_time < expected_max_time
    ...
    def test_fitness_function_call_efficiency(self):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=counting_fitness, param_bounds=bounds, config=config
        )
        ...
        assert (
            min_expected_calls <= call_count <= max_expected_calls
...
class TestParallelizationPotential:
    ...
    def test_independent_fitness_evaluations(self):
        ...
        optimizer = GeneticOptimizer(
            fitness_function=order_tracking_fitness, param_bounds=bounds, config=config
        )
        ...
        assert len(set(evaluation_order)) == len(
            evaluation_order
...
class TestRealTimePerformance:
    ...
    def test_pharmaceutical_real_time_requirements(self):
        ...
        def pharmaceutical_fitness(control_plan):
            ...
            control_effort = (
                np.sum(np.diff(control_plan, axis=0) ** 2) if control_plan.shape[0] > 1 else 0
            )
        ...
        optimizer = GeneticOptimizer(
            fitness_function=pharmaceutical_fitness, param_bounds=bounds, config=config
        )
        ...
        print(
            f"Pharmaceutical MPC optimization: {execution_time:.2f}s, fitness: {final_fitness:.2f}"
        )
    ...
    def test_repeated_optimization_performance(self):
        ...
        for iteration in range(num_iterations):
            optimizer = GeneticOptimizer(
                fitness_function=simple_mpc_fitness, param_bounds=bounds, config=config
            )
        ...
        print(
            f"Repeated optimization: avg={avg_time:.2f}s, min={min_time:.2f}s, max={max_time:.2f}s"
        )
