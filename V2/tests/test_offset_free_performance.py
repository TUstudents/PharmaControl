...
def test_offset_free_performance_comparison():
    ...
    class MockModelWithDisturbance:
        ...
        def predict_distribution(self, past_cmas, past_cpps, future_cpps, n_samples=30):
            ...
            batch_size, horizon, cma_features = 1, future_cpps.shape[1], len(self.disturbance_offset)
    ...
    for step in range(num_steps):
        ...
        true_disturbance_effect = step_disturbance * 0.9  # 90% of disturbance shows up in measurement
        current_measurement = setpoint + true_disturbance_effect - controller.disturbance_estimate * 0.8
        
        # Print progress every 5 steps
        if step % 5 == 0:
            print(f"Step {step:2d}: Error={error}, Disturbance Estimate={controller.disturbance_estimate}")
...
def test_offset_scaling_mathematical_verification():
    ...
    class MockEstimator:
        def estimate(self, measurement, control): return measurement
