...
def get_high_variability_scenario() -> PharmaceuticalScenario:
    ...
    return PharmaceuticalScenario(
        ...
        time_horizon=10,
        fitness_weights={'tracking': 5.0, 'control_effort': 0.3, 'constraints': 1000.0}  # High tracking weight
    )
...
def get_constrained_operation_scenario() -> PharmaceuticalScenario:
    ...
    return PharmaceuticalScenario(
        ...
        time_horizon=10,
        fitness_weights={'tracking': 1.0, 'control_effort': 0.1, 'constraints': 5000.0}  # High constraint penalty
    )
...
def create_fitness_function_from_scenario(scenario: PharmaceuticalScenario) -> Callable[[np.ndarray], float]:
    """
    ...
    def scenario_fitness(control_plan: np.ndarray) -> float:
        ...
        total_cost = (weights['tracking'] * tracking_cost +
...
def _granulation_process_model(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> np.ndarray:
...
def _coating_process_model(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> np.ndarray:
    ...
    coating_time = control_plan[:, 4] if control_plan.shape[1] > 4 else np.ones_like(spray_rate) * 120
...
def _tableting_process_model(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> np.ndarray:
...
def _generic_process_model(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> np.ndarray:
...
def _calculate_tracking_cost(predicted_outputs: np.ndarray, scenario: PharmaceuticalScenario) -> float:
...
def _calculate_control_effort_cost(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> float:
...
def _calculate_constraint_penalty(control_plan: np.ndarray, scenario: PharmaceuticalScenario) -> float:
...
def generate_test_control_sequence(scenario: PharmaceuticalScenario, 
                                 sequence_type: str = 'random') -> np.ndarray:
    """
...
def validate_pharmaceutical_constraints(control_plan: np.ndarray, 
                                      scenario: PharmaceuticalScenario) -> Dict[str, bool]:
    """
