...
class V1ControllerAdapter:
    ...
    def _calculate_soft_sensors(self, cpps: Dict[str, float]) -> Dict[str, float]:
        ...
        if not np.isfinite(carousel_speed) or carousel_speed <= 0:
            raise ValueError(f"Invalid carousel_speed: {carousel_speed} (must be finite and positive)")
    ...
    def _build_dataframes(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple[pd.DataFrame, pd.DataFrame]: (past_cmas_df, past_cpps_df) for V1 interface
        ...
        past_cmas_df = full_df[self.cma_names]
        past_cpps_df = full_df[self.cpp_full_names]
    ...
    def suggest_action(self, current_cmas: Dict[str, float], current_cpps: Dict[str, float], 
        ...
        try:
            ...
            # Call V1 controller with proper DataFrame interface
