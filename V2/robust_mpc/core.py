...
class RobustMPCController:
    ...
    def _get_fitness_function(self, past_cmas_scaled, past_cpps_scaled, target_cmas_unscaled):
        ...
        def fitness(control_plan_scaled):
            ...
            past_cmas_tensor = (
                torch.tensor(past_cmas_scaled, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
            past_cpps_tensor = (
                torch.tensor(past_cpps_scaled, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
            future_cpps_tensor = (
                torch.tensor(plan_scaled, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
            ...
            corrected_mean_scaled = mean_pred_scaled + torch.tensor(
                disturbance_scaled, device=self.device
            )

            # Adjust for risk (Uncertainty-Awareness)
            beta = self.config.get(
                "risk_beta", 1.0
            )  # FIXED: Reduced from 1.5 to 1.0 for balanced exploration
    ...
    def suggest_action(self, noisy_measurement, control_input, setpoint, timestamp=None):
        ...
        if len(self.history_buffer) > 0:
            ...
            if cma_len != cpp_len:
                if self.config.get("verbose", False):
                    print(
                        f"CRITICAL: Buffer misalignment detected - CMA: {cma_len}, CPP: {cpp_len}"
                    )
        ...
        if self._should_reset_optimizer(setpoint):
            if self.config.get("verbose", False):
                print(
                    f"Significant setpoint change detected, resetting optimizer for fresh exploration"
                )
    ...
    def _get_real_history(self):
        ...
        if self.history_buffer.is_ready(lookback):
            ...
            if available_samples > 0:
                ...
                startup_cmas, startup_cpps = self.startup_generator.generate_startup_history(
                    missing_samples
                )
                ...
                past_cmas_unscaled, past_cpps_unscaled = (
                    self.startup_generator.generate_startup_history(lookback)
                )
    ...
    def _scale_cpp_plan(self, plan_unscaled, with_soft_sensors=False):
        ...
        if with_soft_sensors:
            ...
            if missing_base:
                raise ValueError(
                    f"Missing required base variables for soft sensors: {missing_base}"
                )
            if missing_soft:
            ...
            plan_df = pd.DataFrame(
                data=np.zeros((horizon, len(cpp_full_names))), columns=cpp_full_names
            )
            ...
            try:
                ...
                plan_df["specific_energy"] = (
                    plan_df["spray_rate"] * plan_df["carousel_speed"]
                ) / 1000.0
    ...
    def _calculate_safe_default_action(self):
        ...
        for i, name in enumerate(self.config["cpp_names"]):
            if name in cpp_config:
                ...
                if min_val >= max_val:
                    raise ValueError(
                        f"Invalid constraint bounds for '{name}': min_val={min_val} >= max_val={max_val}"
                    )
                ...
                raise ValueError(
                    f"Missing constraint configuration for CPP '{name}' - required for safe fallback"
                )
    ...
    def _validate_initialization(self):
        ...
        if (
            not isinstance(self.config["cpp_full_names"], list)
            or len(self.config["cpp_full_names"]) == 0
        ):
        ...
        missing_soft_base = [
            var for var in required_soft_sensor_base if var not in self.config["cpp_full_names"]
        ]
        missing_soft_sensors = [
            var for var in required_soft_sensors if var not in self.config["cpp_full_names"]
        ]

        if missing_soft_base:
            raise ValueError(
                f"Missing required base variables for soft sensor calculations: {missing_soft_base}"
            )
        if missing_soft_sensors:
            raise ValueError(
                f"Missing required soft sensor variables in cpp_full_names: {missing_soft_sensors}"
            )
    ...
    def _validate_scaling_inputs(self, data, expected_shape_desc, method_name):
        ...
        if self.config.get("verbose", False):
            print(
                f"{method_name} input validation passed: shape {data.shape} ({expected_shape_desc})"
            )
