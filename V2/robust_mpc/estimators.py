...
class KalmanStateEstimator:
    ...
    def __init__(
        ...
        self.kf = KalmanFilter(
            ...
            * process_noise_std**2,  # How much we trust the model
            observation_matrices=np.eye(n_dim_state),  # We directly measure the state
            observation_covariance=np.eye(n_dim_state)
            * measurement_noise_std**2,  # How much we trust the measurement
...
class BiasAugmentedKalmanStateEstimator:
    ...
    def __init__(
        ...
        A_aug[: self.n_physical_states, : self.n_physical_states] = (
            transition_matrix  # A for physical states
        )
        A_aug[: self.n_physical_states, self.n_physical_states :] = np.eye(
            self.n_physical_states
        )  # Bias affects physical state evolution
        A_aug[self.n_physical_states :, self.n_physical_states :] = np.eye(
            self.n_physical_states
        ...
        C_aug[:, : self.n_physical_states] = np.eye(
            self.n_physical_states
        )  # Observe physical state only
        ...
        Q_aug[: self.n_physical_states, : self.n_physical_states] = (
            np.eye(self.n_physical_states) * process_noise_std**2
        )
        Q_aug[self.n_physical_states :, self.n_physical_states :] = (
            np.eye(self.n_physical_states) * bias_process_noise_std**2
        )
        ...
        initial_augmented_cov[
            : self.n_physical_states, : self.n_physical_states
        ] *= 1.0  # Low uncertainty for physical states
        initial_augmented_cov[
            self.n_physical_states :, self.n_physical_states :
        ] *= 100.0  # High uncertainty for unknown bias
