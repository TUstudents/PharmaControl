...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "kalman_filter": {"process_noise_std": 1.0, "measurement_noise_std": 15.0},
        "mpc": {
            "lookback": 36,
            "horizon": 50,
            "integral_gain": 0.05,
...
class MockPlantSimulator:
    ...
    def step(self, cpps):
        ...
        self.state["d50"] = max(
            300, min(500, self.state["d50"] * 0.98 + spray_effect + np.random.normal(0, 2))
        )
        self.state["lod"] = max(
            0.5, min(3.0, self.state["lod"] * 0.95 + air_effect * 0.1 + np.random.normal(0, 0.05))
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--steps", type=int, default=None, help="Number of control steps to run (overrides config)"
    )
    parser.add_argument(
        "--no-realtime",
        action="store_true",
